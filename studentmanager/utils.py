...
def is_valid_ssn(ssn, date_of_birth):
    ...
    The function checks: length, format and allowed characters; indication of the date of birth; century character; control character.
...
def generate_control_character(ssn):
    ...
    :param ssn: string representing the partial ssn (characters 1-10) for which to generate the control character
...
def generate_ssn(date):
    """
    Generate a valid ssn from the fiven date of birth. The sequential section (characters 8-10) are random.
    :param date: datetime.date object indicating the date of birth for which to generate a valid ssn
